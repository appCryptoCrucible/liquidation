...
Last updated: 2026-09-19T17:35Z (02A built; D58 prefetch policy set; 02A under Opus review; C1 building)
...
- **02A / target hardware (re-measure + prefetch future):** the 1k-sweep 50 µs budget was not met on the 2016 i5-7300U test laptop (256 KB L2; ~1.3 MB working set spills to L3; 21 µs when L2-resident). Per **D58**, re-measure on the target HFT box before treating 50 µs as pass/fail. `prefetcht0` is a documented future optimization (blocked by `unsafe_code = "forbid"`; revisit only if target hardware also misses). (Surfaced by the 02A builder; D58.)
