...
| **02A** | State store core | GUIDE 02 §1, §2, §5, §6, §7b | `crates/liq-state/src/{interner,store,undo,view}.rs` | 01 | **T1 → Opus** | Column-major flat `Vec<u128>` per slot; `AssetMask` first; `impl StateWriter`; undo ring `Box<[BlockUndo; K]>` pre-reserved; `undo(apply(x)) == x` proptest over random sequences incl. `Created`; deep-reorg returns `ReorgTooDeep`, never partial; `StateView` + `Overlay`; grep-assert no `Arc/Mutex/RwLock/atomic` in the definition; ≤ 12 cache lines / 1,000 positions < 50 µs / 128-block unwind < 5 ms measured; mutation #8 red |
...
| **13A** | Execution | GUIDE 13 §1–§5; D18–D20 | `crates/liq-exec/`, `config/builders.toml` | **G-S passed**, H3, 12A-2, 06B, 14A, W | **T1 → Opus** | `Submitter` impls: `MevShare` (body `[{hash}, {tx}]`, `inclusion` spanning 2–3 blocks, `refundConfig`) and `BuilderBundle` (curated endpoints, `JoinSet` fan-out); **no `PublicMempool` variant exists** (D54); identity-key header signing from 06B; presigned templates with patch points; `NonceAllocator` — `parking_lot::Mutex` per key, guard dropped before any `.await`, gap filler; fee fields from the gas oracle per submission; hot → exec by `try_send`, counted; `RiskGate.allow()` before every send; inclusion watcher → `Terminal`, `LostToCompetitor` resolved via W's stream with the winner's bid ≥ 90 %; 20-slot synthetic cascade with no stall or gap; sign + submit p99 < 500 µs |
...
| **15C-\<proto\>** | One WP per Tier 2/3 adapter | GUIDE 15 §3 checklist | `crates/liq-adapters/<proto>/`, `docs/coverage/<proto>.md`, fixtures, `config/protocols/<proto>.toml` | 15B, 15A-1 | T2 → T1 | Full per-adapter checklist through drift. **If the protocol's liquidation ABI is new to the Executor, this WP also opens `10R-<n>`** — a redeploy WP that repeats 10A–10C and H3 for the added on-chain adapter (D48). Batch several new families into one `10R` |
