...
Live, same block, to the wei: Aave V3/Spark `getAssetPrice` (100 prices, 80 + 20, at block 26054734), Aave V4 spoke `0xe190…` `getReservePrice`, Compound cUSDC `getUnderlyingPrice` (`mantissa / 1000`), Morpho `price()` on 20 pins. Euler, Fluid, Liquity and Gearbox have unit tests of the conversion and still need that same live equality test. The Aave V3 plain-Chainlink inventory is done and added nothing. Derived specs, `AnswerUpdated` wiring, and the same inventory for the other protocols are not done. Gearbox G1 (pull-feed alarm) is on origin (`0e09099`). G2–G6 stay unbuilt until that alarm fires.
