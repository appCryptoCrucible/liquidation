...
`encode` validates then `ProtocolError::ExecutorUnwired` until 10R wires the four ABIs. No `ExecutorAdapter` discriminant. Happy-path Unwired is 10R. Check 9 cannot Ok — inapplicable, not starved with healthy-only fixtures.
