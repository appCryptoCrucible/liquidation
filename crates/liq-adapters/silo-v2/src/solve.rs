//! Last healthy price (`isSolvent` boundary) and accrual crossing.
//! No IRM in events — `time_to_cross` does not invent a rate.
...
/// Largest RAY price that keeps `hf_ray` multiplies inside `U256`.
...
/// Smallest solvent price in `[lo, hi]`, or `None` if the high end is insolvent.
fn smallest_healthy(
...
/// Largest solvent price in `[lo, hi]`, or `None` if the low end is insolvent.
...
pub(crate) fn liquidation_price(
    ...
    if t.debt_assets.is_zero() || t.sum_coll_assets.is_zero() || t.lt.is_zero() {
        return Ok(None);
    }
    let entry =
    ...
    let hi = if is_coll {
        max_price(t.sum_coll_assets, t.coll_row.decimals)?
    } else {
        max_price(t.debt_assets, t.debt_row.decimals)?
    };
    let raw = if is_coll {
